...
${chalk.bold('Note:')} The daemon runs in the background and manages terminal sessions.

${chalk.bold('To clean up runaway processes:')} Use ${chalk.cyan('happy doctor clean')}
`)
    }
    return;
  } else if (subcommand === 'terminal' || subcommand === undefined || subcommand === '--help' || subcommand === '-h') {

    // Running `happy` with no arguments starts a terminal session,
    // exactly like `happy terminal`.

    if (subcommand === '--help' || subcommand === '-h') {
      console.log(`
${chalk.bold('happy')} - Remote terminal, end-to-end encrypted

${chalk.bold('Usage:')}
  happy                   Start an E2E-encrypted remote terminal session
  happy terminal          Same as above
  happy auth              Manage authentication
  happy notify            Send push notification
  happy daemon            Manage background service that allows
                            to spawn new sessions away from your computer
  happy server            Manage server connection settings
  happy doctor            System diagnostics & troubleshooting
  happy bye               Print a farewell message

${chalk.bold('Examples:')}
  happy                    Start a terminal session
...
`)
    ...
    try {
      const { runTerminal } = await import('@/terminal/runTerminal');
    ...
    console.log('Run "happy --help" for usage information.')
    process.exit(1)
  }
})();
